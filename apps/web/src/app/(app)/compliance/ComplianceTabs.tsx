...
export function ComplianceTabs({ active }: { active: "findings" | "coverage" | "reviews" | "packs" }) {
  const tab = (href: string, key: "findings" | "coverage" | "reviews" | "packs", label: string) => (
    <Link
      href={href}
  ...
  return (
    <nav className="flex gap-5 border-b border-subtle">
      ...
      {tab("/compliance/packs", "packs", "Audit packs")}
    </nav>
  );
}
