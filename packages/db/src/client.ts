...
import { serverEnv } from "@dronops/shared/env";
...
function make(url: string): AppDatabase {
...
export function getDb(): AppDatabase {
  if (!_app) _app = make(serverEnv().DATABASE_URL);
  return _app;
}

/** Privileged client (bypasses RLS): jobs and seed only. Never the request path. */
export function getAdminDb(): AppDatabase {
  if (!_admin) _admin = make(serverEnv().ADMIN_DATABASE_URL);
  return _admin;
}
