...
export function tenantIsolationSuite({ table, extraColumns = {} }: IsolationCase) {
  const url = process.env.DATABASE_URL;
  const cols = Object.keys(extraColumns);
  const colList = ["org_id", ...cols].map((c) => `"${c}"`).join(", ");
  const valList = (org: string) => [`'${org}'::uuid`, ...cols.map((c) => extraColumns[c])].join(", ");
  ...
  describe.skipIf(!url)(`tenant isolation: ${table}`, () => {
    ...
    it("isolates select + insert and forbids delete for app_user", async () => {
      // The whole scenario runs in one DO block so the role/GUC context holds.
      const rows = await sql.unsafe<Array<{ ok: boolean }>>(`
        DO $$
        DECLARE visible int; blocked boolean;
        BEGIN
          INSERT INTO ${table}(${colList}) VALUES (${valList(ORG_A)});
          INSERT INTO ${table}(${colList}) VALUES (${valList(ORG_B)});
          ...
          blocked := false;
          BEGIN INSERT INTO ${table}(${colList}) VALUES (${valList(ORG_B)});
